...
DATA_PATH = config["data"].get("raw_path", "data/bank.csv")

TEST_SIZE = config["data"].get("test_size", 0.2)
RANDOM_STATE = config["project"].get("random_state", 42)

MLFLOW_URI = config["mlflow"].get("tracking_uri")
EXPERIMENT_NAME = config["mlflow"].get("experiment_name", "bank_churn_model")
...
def main():
    ...
    with mlflow.start_run(run_name=f"train_{timestamp}"):
        ...
        latest_model = os.path.join(MODEL_PATH, "model.pkl")
